...
{
	char c = 0;
	unsigned char data , addr_block, addr_high, addr_low;
	unsigned char wave;
	I2CInitial();
	
	while (c != 'q')
	{
		printf("\r\n1 Byte write to EEPROM");
		printf("\r\n2 Byte read from EEPROM");
		printf("\r\n3 128b block sequential write to EEPROM");
		printf("\r\n4 128b block sequential read from EEPROM");
		printf("\r\n5 sequential write any size");
		printf("\r\n6 sequential read any size");
		printf("\r\n7 Generate Waveform");
		printf("\r\n8 read ADC channel 0");
		printf("\r\nq quit\r\n");
		c = tolower(_getch());
		switch (c)
		{
		...
		case '6':
			...
			break;
		case '7':
			printf("\r\nNumber of wave in hex 2\r\n");
			wave = Get2HexDigits(0);
			I2CWriteDAC(wave);
			break;
		case '8':
			I2CReadADC();
...
void ProgramFlashChip(void)
{
	...
	//EraseFlashChip(FlashPtr);
    //erase first 64 k block as 8 sectors of 8k each
    for(i = 0; i < 8; i++)
        FlashSectorErase( i, FlashPtr );

	
    //erase next sectors of 64 k block
	//Num_FlashSectors=3
    for(i = 1; i < Num_FlashSectors + 1 ; i++)
        FlashSectorErase( i << 3, FlashPtr );
	
    printf("\r\nProgramming Flash Memory.....") ;
	//for debuging
	/*for (i = 0; i < FlashSize; i++) {
		if (FlashRead(i, FlashPtr) != 0xff) {
			printf("\r\n[%08x] : %02x  ", i , FlashRead(i, FlashPtr));
			return;
		}
	}*/
	
    for(i = 0; i < FlashSize+1; i ++) {   // i = address offset to the Flash chip
        FlashProgram(i, *RamPtr++, FlashPtr) ;    // address offset into flash, byte data
    }
    ...
    for(i = 0; i < FlashSize; i ++) {
        if(FlashRead(i, FlashPtr) != *RamPtr++) {

			printf("\r\n[%08x] : %02x  ", RamPtr, *RamPtr);
			printf("\r\n[%08x] : %02x  ", i<<1, FlashRead(i, FlashPtr));
			RamPtr--;
			printf("\r\nFAILED.....") ;
            return ;
        }
...
char MemoryTestInt(unsigned int **RamPtr, unsigned int i, unsigned int terminate)
{
	int j;
	while (*RamPtr < (terminate))
	{
		**RamPtr = i;
		if (**RamPtr != i)
			for (j = 0; j < 4; j++)
                //j<<3 gives 8*j, *RamPtr>>8*j are *RamPtr, *RamPtr>>8, *RamPtr>>16, *RamPtr>>24 respectively
				if ((((**RamPtr) >> (j << 3)) & 0xff) != ((i >> (j << 3)) & 0xff))
				{
					printf("\r\nError at Address [%08x], Wrote[%02x], Read [%02x]", (unsigned int)*RamPtr + j, (i >> (j << 3)) & 0xff, ((**RamPtr) >> (j << 3)) & 0xff);
					return 1;
				}
		*RamPtr=*RamPtr+1;
	}
...
char MemoryTestByte(unsigned int **RamPtr, unsigned int i, unsigned int remain)
{
	int j;
	for (j = 0; j < remain; j++)
	{
		**RamPtr = (i >> (j << 3)) & 0xff;
		if (**RamPtr != ((i >> (j << 3)) & 0xff))
		{
			printf("\r\nError at Address [%08x], Wrote[%02x], Read [%02x]", *RamPtr, (i >> (j << 3)) & 0xff, **RamPtr);
			return 1;
		}
		*RamPtr = ((unsigned int)*RamPtr)+1; //Move pointer up by 1
	}
	return 0;
}
...
void MemoryTest(void)
{
    ...
    if ( (!((Start >=0x00010000) && (End <=0x00013FFF))) && (!((Start >=0x00820000) && (End <=0x0083FFFF))))
	{
        printf("\r\nPlease enter address from valid address range: 00010000-00013FFF and 00820000-0083FFFF");
        return;
    }
	for (c = 1; c < 4; c++)
	{
		...
		printf("\r\nDoing Merrory Test %u...", c);
		if (Start % 2 == 0)//Even Starting address
		{
			remain = (End + 1 - Start) % 4;
			flag = MemoryTestInt(&RamPtr, i, End-remain) || flag;
		}
		else//Odd Starting address
		{
			remain = (End - Start) % 4;
			flag = MemoryTestByte(&RamPtr, i, 1) || flag;
			flag = MemoryTestInt(&RamPtr, i, End-remain) || flag;
		}
		if (remain != 0)
		{
			flag = MemoryTestByte(&RamPtr, i, remain) || flag;
		}
		if (flag == 0)
			printf("\r\nPassed Memory Test %u", c);
		else
			flag = 0;
	}
	return;
